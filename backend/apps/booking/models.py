...
class Appointment(models.Model):
    ...
    # staff actions
    assigned_staff = models.ForeignKey(User, on_delete=models.SET_NULL, null=True,blank=True, related_name="staff")
...
class RepairLog(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    appointment = models.OneToOneField(Appointment, on_delete=models.CASCADE, related_name="logs", editable=False)
    recorded_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, related_name="assigned")
    
    device_type = models.CharField(max_length=100, help_text="e.g. Laptop, Desktop, Phone", blank=True)
    findings = models.TextField(blank=True)
    remarks = models.TextField(blank=True)
    quoted_amount = models.DecimalField(max_digits=10, decimal_places=2, default=0.00)
    status_at_time = models.CharField(max_length=30, help_text="Snapshot of appointment status when logged")
